...
async def test_zone_methods_and_services(
    ...
    # 2. set_preset_mode
    with patch.object(zone, "async_set_zone_mode") as mock_set:
        zone.set_preset_mode(PRESET_NONE)
    ...
    with patch.object(zone, "async_set_zone_mode") as mock_set:
        # A. No args -> Schedule
        zone.set_temperature(temperature=None)
        mock_set.assert_called_with(
        ...
        zone.set_temperature(temperature=21.0, duration=dur)
        mock_set.assert_called_with(
        ...
        zone.set_temperature(temperature=21.0, until=until)
        mock_set.assert_called_with(
